...
public class Run {
    ...
    // 这个是暂定的问题存放路径
    public static final String PROBLEM_SET_PATH = "/problem";
    ...
    Run(Integer time, String inID, String inProblemID, boolean hasIn, Integer maxMemory, String language) {
        timeout = time;
        submitID = inID;
        problemID = inProblemID;
        hasInput = hasIn;
        this.maxMemory = maxMemory;
        compiledFile = new File(CompileMain.INITIAL_FILE_ADDRESS + "/" + submitID + "/" + submitID + language);
    }
    ...
    public boolean runIt() throws InterruptedException {
        if (hasInput) {     // 如果有输入文件
            ...
            for (Integer i = 0; i < inputFileList.length; i++) {
                File outputFile = new File("/submit/" + submitID + "/out/" + i.toString() + ".out");
                try {
            ...
            File outputFile = new File("/submit/" + submitID + "/out/0.out");
            controllerCodeList = new JudgeSystemConstant[1];
            try {
                outputFile.createNewFile();
                dealTask(outputFile, null, 0);
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
        // 将结果列表写入数据库
        while (!dealTaskThreadPool.isShutdown()) {
            Thread.sleep(100);
        }
        int ACs = 0;
        JudgeSystemConstant finalStatus = JudgeSystemConstant.WAITING;
        for (JudgeSystemConstant i : controllerCodeList) {
            switch (i) {
                case AC:
                    ACs++;
                    break;
                case MLE:
                    finalStatus = JudgeSystemConstant.MLE;
                    break;
                case RE:
                    finalStatus = JudgeSystemConstant.RE;
                    break;
                case TLE:
                    finalStatus = JudgeSystemConstant.TLE;
                    break;
                case WA:
                    finalStatus = JudgeSystemConstant.WA;
                    break;
                case PE:
                    finalStatus = JudgeSystemConstant.PE;
                    break;
                default:

            }
        }
        if (ACs == controllerCodeList.length) {
            finalStatus = JudgeSystemConstant.AC;
        }

        ProblemStatus status = new ProblemStatus();
        Integer submitIdInt = Integer.getInteger(submitID);
        status.UpdateStatus(submitIdInt, finalStatus);
        status.UpdateRunMemory(submitIdInt, usedMemory);
        status.UpdateRuntime(submitIdInt, usedTime);
